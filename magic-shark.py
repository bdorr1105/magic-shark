...
# Define the function to scan top traffic
def scan_top_traffic(pcap_file):
    # Run the tshark command to extract source IP addresses, source ports, destination IP addresses, and destination ports
    timestamps = subprocess.check_output(["tshark", "-r", pcap_file, "-T", "fields", "-e", "frame.time"], stderr=subprocess.DEVNULL).decode().splitlines()
    source_ips = subprocess.check_output(["tshark", "-r", pcap_file, "-T", "fields", "-e", "ip.src"], stderr=subprocess.DEVNULL).decode().splitlines()
    source_ports = subprocess.check_output(["tshark", "-r", pcap_file, "-T", "fields", "-e", "tcp.srcport", "-e", "udp.srcport"], stderr=subprocess.DEVNULL).decode().splitlines()
    dest_ips = subprocess.check_output(["tshark", "-r", pcap_file, "-T", "fields", "-e", "ip.dst"], stderr=subprocess.DEVNULL).decode().splitlines()
    dest_ports = subprocess.check_output(["tshark", "-r", pcap_file, "-T", "fields", "-e", "tcp.dstport", "-e", "udp.dstport"], stderr=subprocess.DEVNULL).decode().splitlines()

    # Count the occurrences of each source IP and port to destination IP and port
    traffic_counts = {}
    for i in range(len(source_ips)):
        timestamp = timestamps[i]
        # Parse and reformat the timestamp
        timestamp_dt = datetime.strptime(timestamp, "%b %d, %Y %H:%M:%S.%f000 %Z")
        formatted_timestamp = timestamp_dt.strftime("%b %d, %Y %H:%M:%S")
        source_ip = source_ips[i]
        source_port = source_ports[i].strip() if source_ports[i].strip() != "" else "-"
        dest_ip = dest_ips[i]
        dest_port = dest_ports[i].strip() if dest_ports[i].strip() != "" else "-"
        key = (source_ip, source_port, dest_ip, dest_port)
        if key not in traffic_counts:
            traffic_counts[key] = [1, formatted_timestamp]
        else:
            traffic_counts[key][0] += 1

    # Sort the traffic counts by count
    sorted_traffic = sorted(traffic_counts.items(), key=lambda x: x[1][0], reverse=True)

    # Display the top traffic counts
    print()
    print("{:<8} {:<8} {:<25} {:<18} {:<10} {:<5} {:<18} {:<13}".format("Rank", "Count", "Timestamp", "Source IP", "Source Port", "-->", "Destination IP", "Destination Port"))
    for i, item in enumerate(sorted_traffic):
        count = item[1][0]
        timestamp = item[1][1]
        source_ip = item[0][0]
        ...
        dest_ip = item[0][2]
        dest_port = item[0][3]
        print("{:<8} {:<8} {:<25} {:<20} {:<10} {:<5} {:<20} {:<15}".format(i+1, count, timestamp, source_ip, source_port, "-->", dest_ip, dest_port))
