...
def scan_top_traffic(pcap_file):
    ...
    sorted_traffic = sorted(traffic_counts.items(), key=lambda x: x[1], reverse=True)

    # Display the top traffic counts
    print()
    print("{:<8} {:<8} {:<15} {:<15} {:<15} {:<15}".format("Rank", "Count", "Source IP", "Source Port", "Destination IP", "Destination Port"))
    for i, item in enumerate(sorted_traffic):
        source_ip = item[0][0]
        source_port = item[0][1]
        dest_ip = item[0][2]
        dest_port = item[0][3]
        print("{:<8} {:<8} {:<15} {:<15} {:<15} {:<15}".format(i+1, item[1], source_ip, source_port, dest_ip, dest_port))
